...
class UpdatedYTDAuditEngine:
    ...
    def audit_invoice(self, invoice_no: str) -> Dict:
        ...
        try:
            if transportation_mode in ['air']:
                ...
                # Don't add error for template compatibility - use overall_status instead
                
        except Exception as e:
            # For unexpected errors, set minimal error structure compatible with template
    ...
    def _convert_air_audit_result(self, air_result: Dict, invoice_data: Dict) -> Dict:
        ...
        # Extract data safely from air audit result
        charge_breakdown = air_result.get('charge_breakdown', [])
        rate_card_info = air_result.get('rate_card_info', {})
        invoice_audit_data = air_result.get('invoice_data', {})
        
        # Handle the case where invoice_data might be a string instead of dict
        if isinstance(invoice_audit_data, str):
            invoice_audit_data = {}
        ...
        audit_result = {
            ...
            'total_expected': invoice_audit_data.get('total_expected_usd', 0),
            'total_actual': invoice_audit_data.get('total_actual_usd', 0),
            'total_variance': invoice_audit_data.get('total_variance_usd', 0),
            ...
            'status_reason': air_result.get('reason', ''),
            'rate_card_info': {
    ...
    def _convert_ocean_audit_result(self, ocean_result: Dict, invoice_data: Dict) -> Dict:
        ...
        audit_result = {
            ...
            'status_reason': ocean_result.get('reason', ''),
            'rate_card_info': {
    ...
    def _convert_air_charge_breakdown(self, charge_breakdown: Dict) -> List[Dict]:
        """Convert air freight charge breakdown to unified format"""
        variances = []
        
        # Air audit returns a dict where keys are charge types
        for charge_type, charge_data in charge_breakdown.items():
            if isinstance(charge_data, dict):
                variances.append({
                    'charge_type': charge_type.replace('_', ' ').title(),
                    'expected': charge_data.get('rate_card_amount_usd', 0),
                    'actual': charge_data.get('invoice_amount_usd', 0),
                    'variance': charge_data.get('variance_usd', 0),
                    'variance_pct': charge_data.get('percentage_variance', 0),
                    'analysis': charge_data.get('audit_type', '')
                })
        
        return variances
    
    def _convert_ocean_charge_breakdown(self, charge_breakdown) -> List[Dict]:
        """Convert ocean freight charge breakdown to unified format"""
        variances = []
        
        if not charge_breakdown:
            return variances
            
        # Handle both dict and list formats
        if isinstance(charge_breakdown, dict):
            # Ocean audit returns a dict where keys are charge types
            for charge_type, charge_data in charge_breakdown.items():
                if isinstance(charge_data, dict):
                    variances.append({
                        'charge_type': charge_type.replace('_', ' ').title(),
        ...
        elif isinstance(charge_breakdown, list):
            # Handle list format if returned by ocean engine
...
def audit_invoice(invoice_no):
    ...
    # Always pass the results object to the template
    # The template can handle error states within the results structure
